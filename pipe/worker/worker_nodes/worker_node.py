...
class WorkerNode:
    ...
    def listen(self):
        ...
        while True:
            ...
            logger.info(f"{msg=}")
            try:
                if msg[const.CMD] == const.MAIN_CONNECTION_ESTABLISHED:
                    response = utils.make_cmd(const.WORKER_CONNECTION_ESTABLISHED_ACK, {})
                    self.socket.send_json(response)
                elif msg[const.CMD] == const.MAIN_TASK:
                    data = msg[const.DATA]
                    func = TASK_REGISTRY.get(data['name'], 'not_found')
                    args = data.get('args', list())
                    kwargs = data.get('kwargs', dict())
                    self.add_task(func, *args, **kwargs)
                else:
                    logger.error(f"Undefined CMD: {msg[const.CMD]}.")
...
if __name__ == '__main__':
    ...
    # Example usage
    worker = WorkerNode(num_workers=4)  # Specify the number of worker threads
